 ...
 */
function Node(oid,code,name,type,level,open,codeOrder,chiefJobOid,chiefJobName,managerName,historyOid) {
  this.oid = oid;
  ...
  this.chiefJobOid = chiefJobOid;
  //this.chiefJobCode = chiefJobCode;
  this.chiefJobName = chiefJobName;
...
function NodeCreateViewNodeT(oid){
    ...
    if( isCEO ){//panhe modified this function on 2.29.2004(���Ǽ���)
...
var typeKeyValue = "type";
//var noJobMessage = "no chief Job"
var noJobMessage = ""

var chartVersion = "";
var structureOid = "";
var hiberarchyOid = "";
var hiberarchyType = "organizational";
var isCompact=true;
var mode="chart";
...
function doOpen(viewNode){
   ...
   adaptPosition();
   chartInnerText.innerText = chartTd.innerHTML;
   hideActionbar();
}
...
function getUnitTypeLabel(unitType){
   var i,select;
   select = window.allUnitType;
   for(i=0;i<select.options.length;i++){
      if(select.options[i].value==unitType)
         return select.options[i].innerText;
   }
   return "unDefine";
}