...
function user_loading_calendar(id,lan){
	aim_div="#"+id;
	language=lan;
	if(lan=="cn"){
		month_arry=month_cn;
...
(function() {
	window.onload=function(){
		...
		if (id!=null&&id!=""){
			$.ajax({
				type:"POST",
				url:"/admin/user/findUsersWorkInfo",
				data:{"id":id},
				dataType:"JSON",
							...
							success:function (data) {
								if (data!=null&&data!=""){
									...
									info="\t\t<div class=\"modal-content\">\n" +
										...
										"\t\t\t\t\t\t\t\t<input type=\"text\" class=\"form-control\"  readonly='readonly'  name='name' value="+work.user.name+" id=\"recipient-name\">\n" +
										"\t\t\t\t\t\t\t</div>\n" +
										"\t\t\t\t\t\t\t<div class=\"form-group\">\n" +
