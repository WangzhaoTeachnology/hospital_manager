...
function _loading_calendar(id,lan){
	aim_div="#"+id;
	language=lan;
	if(lan=="cn"){
		month_arry=month_cn;
...
(function() {
	window.onload=function(){
		...
		if (id!=null&&id!=""){
			$.ajax({
				type:"POST",
				url:"/admin/doctor/findDoctorsWorkInfo",
				data:{"id":id},
				dataType:"JSON",
			...
			$(this).mouseover(function () {
				if ($(this).find("input").length>0){
					...
					dest.click(function () {
						$.ajax({
							type:"GET",
							url:"/admin/doctor/findWorkDetailInfoById",
							data:{"id":id},
							dataType:"JSON",
							success:function (data) {
								if (data!=null&&data!=""){
									...
									info="\t\t<div class=\"modal-content\">\n" +
										...
										"\t\t\t\t\t\t\t\t<input type=\"text\" class=\"form-control\"  readonly='readonly'  name='realname' value="+work.doctor.realname+" id=\"recipient-name\">\n" +
										"\t\t\t\t\t\t\t</div>\n" +
										"\t\t\t\t\t\t\t<div class=\"form-group\">\n" +
